import os
#Не трогать, без этого ещё хуже
os.environ["CUDA_VISIBLE_DEVICES"] = "-1"
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3' #Плевать, ничего не выводим.
    ...
    def vectorize(self, text):
        ...
        for token in tokens:
            if token in self._token_indices:
                ...
                print('Что то не то, игнорим', token)
...
def load_data(data_file, word_tokens, pristine_input, pristine_output, batch_size, seq_length=50, seq_step=25):
    global vectorizer
    try:
        with open(data_file, encoding='utf-8',errors="ignore") as input_file:
            text = input_file.read()
    except FileNotFoundError:
        print("Положите input.txt в папку с этим кодом! Сетке не на чем тренироваться!")
        sys.exit(1)
...
def train(model, x, y, x_val, y_val, batch_size, num_epochs):
    ...
    model.fit(x, y, validation_data=validation_data,
                    ...
                    callbacks=callbacks)
    train_end = time.time()
    #print('Время - ', train_end - train_start)
