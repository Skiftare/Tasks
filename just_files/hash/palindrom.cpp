...
bool check(vector<int> &h, vector<int> &rev_h, vector<int> &p_pow, int l,int r){
    int rev_l = n - (r+1);
    int rev_r = n - (l+1);
    int getHash = (h[r] - h[l] + inf)%inf;
    int getRevHash = (rev_h[rev_r] - rev_h[rev_l] + inf)%inf;
    getHash = (getHash * p_pow[rev_l])%inf;
    getRevHash = (getRevHash * p_pow[l])%inf;
...
signed main()
{
    ...
    cout<<max(best, max(2*even_c[n-1], 2*odd_c[n-1]-1))<<endl;
    return 0;
}
